...
- 2.服务器把恶意脚本保存到服务器
- 3.当正常客户访问服务器是，服务器会读取恶意数据并且直接使用
- 4.服务器会返回含有恶意脚本的页面
...
- 伪造请求GET POST 请求
- XSS 钓鱼 通过XSS向网页注入钓鱼链接，让用户访问假冒的网站
### 1.5如何防御XSS
...
### 1.6 Web相关编码和转义
#### 1.6.1 URL编码
- 一般来说，URL只能使用英文字母（a-z A-Z）、数字（0-9）、-_.~4个特殊字符以及所有（;,/?;@&=+$#）保留字符。
...
#### 1.6.2 HTML编码
在HTML中，某些字符是预留的，比如不能使用小于号（<）和大于号（>）,这是因为浏览器会误认为他们是标签。如果希望正确的显示预留字符，我们必须在HTML源代码中使用字符实体 HTML编码分为：
- HTML 十六进制编码 &#xH
- HTML 十进制编码 &#D;
- HTML 实体编码 <等

在HTML进制编码中其中的数字则是对应字符的unicode字符编码。比如单引号unicode字符编码是27，则单引号可以被编码为'
```
